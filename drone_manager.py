...
class DroneManager:
    def __init__(self, detector: CollisionDetectorLIME, use_webcam: bool = False):
        ...
        if not TELLO_AVAILABLE and not use_webcam:
             print("[Manager] Warning: Tello requested but library missing. Forcing webcam mode.")
        ...
        try:
            if self.use_webcam:
                ...
                try:
                    ...
                    print(f"[Manager] Tello connection failed: {e}. Falling back to Webcam.")
                    self.use_webcam = True
                    self._connect_webcam()
    ...
    def _connect_tello(self):
        ...
        self.tello = Tello(); self.tello.connect()
        self.battery_level = self.tello.get_battery()
        if self.battery_level == 0:
             raise RuntimeError("Tello connection established but communication failed (Battery 0%).")
        print(f"[Manager] Tello connected. Battery: {self.battery_level}%")
        self.tello.streamon(); self.frame_reader = self.tello.get_frame_read()
        self.tello.send_rc_control(0, 0, 0, 0); self.is_streaming = True
    ...
    def shutdown(self):
        ...
        if self.tello and not self.use_webcam:
            try:
                ...
                self.tello.send_rc_control(0, 0, 0, 0); self.tello.land(); time.sleep(3)
            except Exception as e:
    ...
    def _get_frame(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            print(f"[Manager] Error getting frame: {e}"); return None
