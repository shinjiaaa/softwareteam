...
class CollisionDetectorLIME:
    def __init__(self, weights_path: Optional[str] = None):
        # (초기화는 이전과 동일)
        self.config = {
            "imgsz": 512, "conf_thres": 0.4, "min_conf_for_lime": 0.6,
            "warning_threshold": 0.75, "roi_shrink": 192, "topk": 1,
            "lime_samples": 150, "lime_alpha": 0.65
        }
    ...
    def process_frame(self, frame_bgr: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        processed_frame, _ = draw_boxes(processed_frame, results, conf_thres=cfg["conf_thres"], names=self.names)
