...
def draw_risk_indicator(frame, max_conf, warning_threshold):
    ...
    # max_conf(가장 높은 객체 신뢰도)에 따라 색상 계산
    if max_conf < 0.5:
...
# output: 각 이미지에 대한 [negative_prob, positive_prob] 반환 -> 확률 형태로 변환
# => 각 segment가 최종 예측에 얼마나 기여하는지 계산하기 위함
...
class CollisionDetectorLIME:
    ...
    # (비동기) 실시간 영상 처리 시 LIME 계산을 백그라운드에서 실행
